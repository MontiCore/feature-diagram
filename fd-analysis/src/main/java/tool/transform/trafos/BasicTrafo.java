...
public class BasicTrafo
  ...
  public void visit(FeatureSymbol feature) {
    ...
    Constraint isSelectedConstraint = new Constraint("bool2int", feature.getName() + "IsSelected",
        feature.getName());
    constraints.add(isSelectedConstraint);
    Constraint isUnselectedConstraint = new Constraint("bool_not",
        feature.getName() + "IsUnselected", feature.getName() + "IsSelected");
  ...
  public void visit(ASTAndGroup andGroup) {
    for (int i = 0; i < andGroup.sizeGroupParts(); i++) {
      ...
      addCardinalFeature(currentGroupparent.getNameSymbol(), childFeature.getNameSymbol(), min,
          max);
    }
  }

  public void visit(ASTOrGroup orGroup) {
    addCardinalGroup(currentGroupparent.getNameSymbol(), orGroup, "1",
  ...
  public void visit(ASTCardinalizedGroup cardinalityGroup) {
    addCardinalGroup(currentGroupparent.getNameSymbol(), cardinalityGroup,
  ...
  private void addCardinalFeature(FeatureSymbol parent, FeatureSymbol child, String min,
    ...
    Constraint constraint = new Constraint("bool_le", child.getName() + "IsSelected",
        parent.getName() + "IsSelected");
    //else child in {min, max}
    Constraint constraint2 = new Constraint("int_le_reif", min, child.getName(),
        parent.getName() + "IsSelected");
  ...
  private void addCardinalGroup(FeatureSymbol parent, ASTFeatureGroup children, String min,
    ...
    Constraint constraint2 = new Constraint("bool_or", helperName2, parent.getName() + "IsSelected",
        "true");
